...
| Path | What it is |
|------|------------|
| `eval/` | Instruction-following meter and safety assessment (`release_meter.py`; 80% execution target, advisory consistency): `tests.md` (Tier 1 — behavioral) and `scenarios/` with `fixtures/` (Tier 3 — real execution, output/state assertions and bounded-delivery probes). Run via `/run-eval` or `/run-probes`; see `eval/README.md` for runtime requirements. |
...
- `eval/results.md` is gitignored (local run history). `eval/tests.md`, `eval/scenarios/`, `eval/fixtures/`, `eval/release_meter.py` and `eval/README.md` are tracked.
