...
- Name scripts in tool/concern families. **The process trigger:** when two scripts are run by hand in a fixed order to produce one output, that is a process — start `cli/{process}/`; do not add a third script to the chain, and do not fold the chain into one big phase-subcommand script (no ceiling, no policy surface, no tests). `scripts/README.md` is a live-scripts table (only live scripts; retired ones drop off)
